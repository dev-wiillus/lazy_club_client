...
type InputProps ={ fields: ControllerRenderProps<any, any>}

export default function ImageUpload(props: InputProps) {
    const {name ='file-upload', ...fields} = props.fields ?? {}
    console.log(props)
    return (
          ...
          <div className="space-y-1 text-center">
            ...
            <div className="text-sm text-gray-600">
              <label htmlFor={name} className="relative cursor-pointer bg-white rounded-md font-medium text-indigo-600 hover:text-indigo-500 focus-within:outline-none focus-within:ring-2 focus-within:ring-offset-2 focus-within:ring-indigo-500">
                ...
                <input id={name} name={name} type="file" className="sr-only" {...fields}/>
              </label>
            </div>
